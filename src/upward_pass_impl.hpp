...
struct base_pots_3d {
	...
	base_pots_3d(view_real& xcos_, view_real& ycos_, view_real& zcos_, view_real& charges_, view_panel_3d& blfmm_panels_, 
				view_reall& proxy_source_weights_, view_int& point_leaf_panel_, int interp_deg_) :
				xcos(xcos_), ycos(ycos_), zcos(zcos_), charges(charges_), blfmm_panels(blfmm_panels_), 
				proxy_source_weights(proxy_source_weights_), point_leaf_panel(point_leaf_panel_), interp_deg(interp_deg_) {}
	...
	void operator()(const int i) const {
		...
		real z = zcos(i);
		int leaf = point_leaf_panel(i);
		real x_basis_vals[max_degree+1], y_basis_vals[max_degree+1], z_basis_vals[max_degree+1];
		real min_x = blfmm_panels(leaf).min_x;
		real max_x = blfmm_panels(leaf).max_x;
		real min_y = blfmm_panels(leaf).min_y;
		real max_y = blfmm_panels(leaf).max_y;
		...
		for (int j = 0; j < interp_deg+1; j++) { // x loop
			for (int k = 0; k < interp_deg+1; k++) { // y loop
				for (int l = 0; l < interp_deg+1; l++) { // z loop
					Kokkos::atomic_add(&proxy_source_weights(leaf,index), x_basis_vals[j]*y_basis_vals[k]*z_basis_vals[l]*charges(i));
					index += 1;
				}
			}
		}
...
struct child_to_parent_3d {
	view_panel_3d blfmm_panels;
	view_reall proxy_source_weights;
	int interp_deg;

	child_to_parent_3d(view_panel_3d& blfmm_panels_, view_reall& proxy_source_weights_, int interp_deg_) : 
					blfmm_panels(blfmm_panels_), proxy_source_weights(proxy_source_weights_), interp_deg(interp_deg_) {}
	...
	void operator()(const int i) const {
		...
		if (parent > -1) {
			...
			real min_y_c = blfmm_panels(i).min_y;
			real max_y_c = blfmm_panels(i).max_y;
			real min_z_c = blfmm_panels(i).min_z;
			real max_z_c = blfmm_panels(i).max_z;
			real min_x_p = blfmm_panels(parent).min_x;
			real max_x_p = blfmm_panels(parent).max_x;
			real min_y_p = blfmm_panels(parent).min_y;
			real max_y_p = blfmm_panels(parent).max_y;
			real min_z_p = blfmm_panels(parent).min_z;
			real max_z_p = blfmm_panels(parent).max_z;
			real cheb_x[max_degree+1], cheb_y[max_degree+1], cheb_z[max_degree+1];
			real x_basis_vals[max_degree+1], y_basis_vals[max_degree+1], z_basis_vals[max_degree+1];
			bli_points_shift(cheb_x, min_x_c, max_x_c, interp_deg);
			bli_points_shift(cheb_y, min_y_c, max_y_c, interp_deg);
			...
			for (int j1 = 0; j1 < interp_deg+1; j1++) { // child x loop
				...
				for (int k1 = 0; k1 < interp_deg+1; k1++) { // child y loop
					interp_vals_bli(y_basis_vals, cheb_y[k1], min_y_p, max_y_p, interp_deg);
					for (int l1 = 0; l1 < interp_deg+1; l1++) {
						interp_vals_bli(z_basis_vals, cheb_z[l1], min_z_p, max_z_p, interp_deg);
						index2 = 0;
						for (int j2 = 0; j2 < interp_deg+1; j2++) { // parent x loop
							for (int k2 = 0; k2 < interp_deg+1; k2++) { // parent y loop
								for (int l2 = 0; l2 < interp_deg+1; l2++) { // parent z loop
									Kokkos::atomic_add(&proxy_source_weights(parent,index2), x_basis_vals[j2]*y_basis_vals[k2]*z_basis_vals[l2]*proxy_source_weights(i,index1));
									index2 += 1;
								}
							}
						}
