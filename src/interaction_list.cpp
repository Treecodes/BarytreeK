...
void dual_tree_traversal_3d(RunConfig& run_config, view_panel_3d_host& blfmm_panels_target, view_panel_3d_host& blfmm_panels_source, view_interact_host& interaction_list) {
	std::vector<interact_pair> temp_interaction_list (0);
	int interaction_count = 0;
	int pp_count = 0;
	int pc_count = 0;
	...
	int index_target, index_source;
	bool refine_target, refine_source;
	real x1, x2, y1, y2, z1, z2, x_ex, y_ex, z_ex, dist, separation;
	...
	while(target_panels.size() > 0) {
		...
		x1 = 0.5*(blfmm_panels_target(index_target).min_x + blfmm_panels_target(index_target).max_x);
		y1 = 0.5*(blfmm_panels_target(index_target).min_y + blfmm_panels_target(index_target).max_y);
		z1 = 0.5*(blfmm_panels_target(index_target).min_z + blfmm_panels_target(index_target).max_z);
		x2 = 0.5*(blfmm_panels_source(index_source).min_x + blfmm_panels_source(index_source).max_x);
		y2 = 0.5*(blfmm_panels_source(index_source).min_y + blfmm_panels_source(index_source).max_y);
		z2 = 0.5*(blfmm_panels_source(index_source).min_z + blfmm_panels_source(index_source).max_z);
		x_ex = x2 - x1;
		y_ex = y2 - y1;
		z_ex = z2 - z1;
		dist = sqrt(x_ex*x_ex + y_ex*y_ex + z_ex*z_ex);
		separation = 100;
		if (dist > 0) {
			separation = (blfmm_panels_target(index_target).radius + blfmm_panels_source(index_source).radius) / dist;
		}
		if (separation < run_config.fmm_theta) {
			...
			if (refine_target) {
				...
				target_panels.push(blfmm_panels_target(index_target).child3);
				target_panels.push(blfmm_panels_target(index_target).child4);
				target_panels.push(blfmm_panels_target(index_target).child5);
				target_panels.push(blfmm_panels_target(index_target).child6);
				target_panels.push(blfmm_panels_target(index_target).child7);
				target_panels.push(blfmm_panels_target(index_target).child8);
			} else if (refine_source) {
				target_panels.push(index_target);
				target_panels.push(index_target);
				target_panels.push(index_target);
				...
				source_panels.push(blfmm_panels_source(index_source).child3);
				source_panels.push(blfmm_panels_source(index_source).child4);
				source_panels.push(blfmm_panels_source(index_source).child5);
				source_panels.push(blfmm_panels_source(index_source).child6);
				source_panels.push(blfmm_panels_source(index_source).child7);
				source_panels.push(blfmm_panels_source(index_source).child8);
			}
		}
	}
	Kokkos::resize(interaction_list, interaction_count);
