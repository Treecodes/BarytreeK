...
struct poisson_pp_interaction_3d {
	view_real xcos_t;
	view_real ycos_t;
	view_real zcos_t;
	view_real xcos_s;
	...
	poisson_pp_interaction_3d(view_real& xcos_t_, view_real& ycos_t_, view_real& zcos_t_, view_real& xcos_s_, view_real& ycos_s_, view_real& zcos_s_, view_real& charges_, 
							view_real& soln_, view_intt& panel_points_inside_target_, view_intt& panel_points_inside_source_, 
							view_interact& interactions_, view_panel_3d& blfmm_panels_target_, view_panel_3d& blfmm_panels_source_, real eps_) :
							xcos_t(xcos_t_), ycos_t(ycos_t_), zcos_t(zcos_t_), xcos_s(xcos_s_), ycos_s(ycos_s_), zcos_s(zcos_s_), charges(charges_), soln(soln_), 
							panel_points_inside_target(panel_points_inside_target_), panel_points_inside_source(panel_points_inside_source_), 
							interaction_list(interactions_), blfmm_panels_target(blfmm_panels_target_), blfmm_panels_source(blfmm_panels_source_), eps(eps_) {}
	...
	void operator()(const int i) const {
		...
		for (int j = 0; j < target_count; j++) {
			...
			for (int k = 0; k < source_count; k++) {
				...
				sy = ycos_s(i_s);
				sz = zcos_s(i_s);
				gfv = gfc/(Kokkos::sqrt((tx-sx)*(tx-sx)+(ty-sy)*(ty-sy)+(tz-sz)*(tz-sz)+eps*eps));
				Kokkos::atomic_add(&soln(i_t), gfv*charges(i_s));
			}
		}
	}
...
struct poisson_pc_interaction_3d {
	view_real xcos_t;
	view_real ycos_t;
	view_real zcos_t;
	view_real soln;
	...
	poisson_pc_interaction_3d(view_real& xcos_t_, view_real& ycos_t_, view_real& zcos_t_, view_real& soln_, view_intt& panel_points_inside_target_, 
							view_reall& proxy_source_weights_, view_interact& interaction_list_, view_panel_3d& blfmm_panels_target_, 
							view_panel_3d& blfmm_panels_source_, real eps_, int interp_deg_) : xcos_t(xcos_t_), ycos_t(ycos_t_), zcos_t(zcos_t_), soln(soln_), 
							panel_points_inside_target(panel_points_inside_target_), proxy_source_weights(proxy_source_weights_), 
							interaction_list(interaction_list_), blfmm_panels_target(blfmm_panels_target_), blfmm_panels_source(blfmm_panels_source_), 
							eps(eps_), interp_deg(interp_deg_) {}
...
struct poisson_cp_interaction_3d {
	view_real xcos_s;
	view_real ycos_s;
	view_real zcos_s;
	view_real charges;
	...
	poisson_cp_interaction_3d(view_real& xcos_s_, view_real& ycos_s_, view_real& zcos_s_, view_real& charges_, view_intt& panel_points_inside_source_, 
							view_reall& proxy_target_weights_, view_interact& interaction_list_, view_panel_3d& blfmm_panels_target_, 
...
struct poisson_cc_interaction_3d {
	view_reall proxy_target_weights;
	view_reall proxy_source_weights;
	view_interact interaction_list;
	view_panel_3d blfmm_panels_target;
	...
	poisson_cc_interaction_3d(view_reall& proxy_target_weights_, view_reall& proxy_source_weights_, view_interact& interaction_list_, 
